...
<body>
    @php
        ...
        $order = $order;
        $invoiceSetting = json_decode(preference('invoice'));
        $logo = App\Models\Preference::where('field', 'company_logo')->first()->fileUrl();
        if ($invoiceSetting?->document?->header?->logo == 'logo' && $invoiceSetting?->general->logo) {
            $logo = App\Models\Preference::where('field', 'invoice')->first()->fileUrl();
        }
    ...
    <div id="invoice-view-container">
        <div id="printTable">
            ...
            <h2>{{ __("Shipment Tracking Information") }}</h2>
            <div>
                <table class="table" id="shipment-tracking">
                    <thead class="thead">
                        <tr>
                    ...
                    <tbody>
                        @if(preference('product_label_wise_shipment_track') == 0)
                            @if($order->shippingTrack)
                                <tr>
                                    <td class="td" width="30%">{{ $order->shippingTrack->provider_name ?? 'No Info' }}</td>
                                    <td class="td" width="20%">{{ $order->shippingTrack->tracking_no ?? 'No Info' }}</td>
                                    <td class="td" width="25%">{{ $order->shippingTrack->order_shipped_date ?? 'No Info' }}</td>
                        ...
                        @if($detail->shippingTrack)
                            <tr>
                                <td class="td" width="40%">{{ $detail->product_name ?? 'No Info' }}</td>
                                <td class="td" width="20%">{{ $detail->shippingTrack->provider_name ?? 'No Info' }}</td>
                                <td class="td" width="10%">{{ $detail->shippingTrack->tracking_no ?? 'No Info' }}</td>
                                <td class="td" width="20%">{{ $detail->shippingTrack->order_shipped_date ?? 'No Info' }}</td>
                                <td class="td" width="25%">
