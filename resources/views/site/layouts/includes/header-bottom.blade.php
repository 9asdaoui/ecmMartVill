<section>
    @php
        $fixedMenu =
            isset($slides) &&
            $slides->count() &&
            isset($header['bottom']['category_expand']) &&
            $header['bottom']['category_expand'] == 1;
            
    @endphp
    @if (
        (isset($header['bottom']['show_page_menu']) && $header['bottom']['show_page_menu'] == 1) ||
    ...
    @endif
    <header
        style="background: {{ $header['bottom']['bg_color'] }}; border-top: 1px solid {{ $header['bottom']['border_top'] }}"
        class="header">
        <div class="flex justify-between layout-wrapper px-4 xl:px-0">
            @if (isset($header['bottom']['show_category']) && $header['bottom']['show_category'] == 1)
                <div class="group group-category dm-sans lg:w-23% md:w-25% h-16 hidden md:block">
                    ...
                    <div
                        class="category-menu-wrapper bg-white border {{ $fixedMenu ? 'transform scale-1 z-30' : 'transform scale-0 group-hover:scale-100 z-40' }} relative h-100 w-full hidden md:block">
                        <div class="accordion-homepage-wrapper">
                            <div
                                class="overflow-hidden tran menu-full-details {{ count($categories) > 8 ? ' height-437p ' : '  ' }} ">
                                @foreach ($categories as $category)
                                    {{-- $usedCategory = $category->is_global ? $category : null; --}}
                                <script> console.log("Category: {{ $category->product_counts}}"); </script>
                                
                                @if ($category->product_counts != 0)
                                    @php $checkChildCategory =count($category->childrenCategories->where('is_global', 1)) @endphp
                                    ...
                                    <div
                                        ...
                                        <div
                                            ...
                                            <a class="flex justify-between text-center items-center">
                                                <span
                                                    class="text-gray-10 font-medium text-sm text-one uppercase ltr:ml-2.5 rtl:mr-2.5">{{ __('See All Categories') }}
                                                </span>
            ...
            @php
                ...
                // Filter out the 'Coupon' menu item if it exists
                $menus = $menus->filter(function ($menu) {
                    return strtolower($menu->label) !== 'coupon';
                });
            @endphp
            <div
                class="{{ isset($header['bottom']['show_category']) && $header['bottom']['show_category'] == 1 ? 'md:ml-5 lg:ml-0 ml-0 md:w-3/4 w-full header-margin-right' : 'w-full ' }}">
                <div class="flex justify-between">
                    @if (isset($header['bottom']['show_page_menu']) && $header['bottom']['show_page_menu'] == 1)
                        <div id="nav-wrap"
                            class="w-full md:w-5/6 lg:w-72% md:mt-3 nav-wrap ltr:mr-0 ltr:md:mr-5 rtl:ml-0 rtl:md:ml-5">
                            <ul class="header-menu-nav text-white md:text-black mx-1 md:mx-1 custom-border">
                                {{-- Solar Estimation Link --}}
                                <li class="first-dropdown-li">
                                    <a style="color: {{ $header['bottom']['text_color'] }}"
                                        class="first-list mb-2 dm-sans text-base custom-bottom-border"
                                        href="{{ route('estimation.index') }}">{{ __('Solar Estimation') }}</a>
                                </li>
                                @foreach ($menus as $menu)
                                    @php
                                        $url = $menu->url(empty($menu->params) ? 'page' : '');
                                        $url =
                                            str_contains($url, url('/')) || str_contains($url, 'http')
                                                ? $url
                                                : url('/' . $url);
                                        $activeUrl = $url;
                                        if (strpos($activeUrl, '?')) {
                                    ...
                                    <li class="first-dropdown-li">
                                        <a style="color: {{ $header['bottom']['text_color'] }}"
                                            class="first-list mb-2 dm-sans text-base custom-bottom-border {{ !empty($menu->class) ? $menu->class : '' }} {{ str_replace('/#', '', $activeUrl) == url()->current() ? 'active-border-bottom' : ' ' }}"
                                        ...
                                        <ul
                                            ...
                                            @foreach ($menu->child as $submenu)
                                                @php
                                                    ...
                                                    $childUrl =
                                                        str_contains($childUrl, url('/')) ||
                                                        str_contains($childUrl, 'http')
                                                            ? $childUrl
                                                            : url('/' . $childUrl);
                                                @endphp
                                                <li class="whitespace-normal border-b break-all">
                                                    <button
                                                        class="outline-none focus:outline-none first-dropdown-menu w-full ltr:text-left rtl:text-right">
                                                        <div
                                                            ...
                                                            <a class="w-48" href="{{ $childUrl }}">
                                                                <div
                                                                    ...
                                                                    @if ($submenu->child->count())
                                                                        <span>
                                                                            <svg class="neg-transition-scale"
                                                                                xmlns="http://www.w3.org/2000/svg"
                                                                                width="5" height="9"
                                                    ...
                                                    <ul
                                                        ...
                                                        @foreach ($submenu->child as $subChildMenu)
                                                            @php
                                                                $subChildUrl = $subChildMenu->url(
                                                                    empty($subChildMenu->params) ? 'page' : '',
                                                                );
                                                                $subChildUrl =
                                                                    str_contains($subChildUrl, url('/')) ||
                                                                    str_contains($subChildUrl, 'http')
                                                                        ? $subChildUrl
                                                                        : url('/' . $subChildUrl);
                                                            @endphp
                                                            <li class="whitespace-normal bg-white border-b break-all">
                                                                    ...
                                                                    <div class="primary-bg-hover">
                                                                        <a class="w-48" href="{{ $subChildUrl }}">
                                                                            <div
                                                                                class="flex items-center justify-between w-full menuss-hover px-15p py-4">
                                                                                <span
                                                                                    class="text-sm cursor-pointer w-36">
                                                                                    {{ $subChildMenu->label }}
                                                                                </span>
                                                                                <span>
                                                                                    <svg class="neg-transition-scale"
                                                                ...
                                                                <ul
                                                                    class="dm-sans text-sm font-medium hidden md:block child-dropdown box-shadow-dropdown">
                                                                    <li
                                                                        class="whitespace-normal bg-white border-b break-all">
                                                                        <button
                                                                            class="outline-none focus:outline-none first-dropdown-menu w-full ltr:text-left rtl:text-right">
                                                                            <div class="primary-bg-hover">
                                                                                <a class="w-48"
                                ...
                                <div
                                    ...
                                    <div>
                                        <div class="relative inline-block ltr:text-left rtl:text-right">
                                            <div style="background: {{ $header['bottom']['bg_color'] }}; color: {{ $header['bottom']['text_color'] }} ;"
                                                class="inline-flex justify-center items-center w-full bg-white text-13 font-medium text-gray-700 cursor-pointer test-click height-63px">
                                                <svg class="ltr:mr-2 rtl:ml-2" width="14" height="20"
                                                    viewBox="0 0 14 20" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <mask id="path-1-outside-1_513_1062" maskUnits="userSpaceOnUse"
                                                        x="2" y="0" width="10" height="6" fill="black">
                                                        <rect fill="white" x="2" width="10" height="6" />
                                                        <path
                                                            d="M8.33333 3H5.66667C5.51167 3 5.43417 3 5.37059 2.98296C5.19804 2.93673 5.06327 2.80196 5.01704 2.62941C5 2.56583 5 2.48833 5 2.33333C5 2.25584 5 2.21709 5.00852 2.1853C5.03164 2.09902 5.09902 2.03164 5.1853 2.00852C5.21709 2 5.25584 2 5.33333 2H8.66667C8.74416 2 8.78291 2 8.8147 2.00852C8.90098 2.03164 8.96836 2.09902 8.99148 2.1853C9 2.21709 9 2.25584 9 2.33333C9 2.48833 9 2.56583 8.98296 2.62941C8.93673 2.80196 8.80196 2.93673 8.62941 2.98296C8.56583 3 8.48833 3 8.33333 3Z" />
                                                    </mask>
                                                    <path
                                                        ...
                                                        fill="{{ $textColor }}"
                                                        mask="url(#path-1-outside-1_513_1062)" />
                                                ...
                                                <svg class="ltr:ml-2.5 rtl:mr-2.5" width="9" height="5"
                ...
                <div class=" flex items-center">
                    <a href="{{ route('estimation.index') }}" class="flex items-center justify-center px-4 py-2 font-medium text-white bg-gray-800 rounded-md hover:bg-gray-700 transition-colors duration-200">
                        <span>Solar Estimation</span>
        ...
        <div class="layout-wrapper px-4 xl:px-0">
            @if (isset($slides) && $slides->count())
                <div class="md:w-3/4 w-full -mt-2 ltr:ml-auto rtl:mr-auto">
                    <div
                        class="slideshow-container md:mt-6 ltr:md:mr-5 ltr:mr-0 ltr:ml-0 ltr:lg:mr-0 rtl:md:ml-5 rtl:ml-0 rtl:mr-0 rtl:lg:ml-0">
                            ...
                            <div class="swiper-wrapper">
                                        ...
                                        <div class="relative z-0 flex align-items-center">
                                            <div class="costume-title w-full px-5 sm:px-11">
                                                <div class="text-{{ $slide->title_direction }}">
                                                    <p class="sliders-animation inline-block anim text-x-title dm-regular animated small-title"
                                                ...
                                                <div class="text-{{ $slide->sub_title_direction }}">
                                                    <p class="sliders-animation inline-block anim text-y-title dm-bold animated bold-title"
                                                ...
                                                @if (!empty($slide->button_title))
                                                    <div
                                                        class="flex {{ $buttonDirection[strtolower($slide->button_position)] }}">
                                                        <a style="animation-delay: {{ $slide->button_delay }}s;"
                                                            ...
                                                            <p class="process-goto shop-btn cursor-pointer relative flex justify-center text-gray-12 rounded-sm md:text-base text-xss items-center md:py-2 py-1.5 w-max md:px-8 px-5 dm-sans mt-2 border md:border-none border-gray-12"
                                                                ...
                                                                {!! $slide->button_title !!}
                                                                <svg class="relative md:w-2.5 md:h-2 w-2 h-6p ltr:ml-5p rtl:mr-2.5 neg-transition-scale"
                                ...
                                <a class="md:flex hidden">
                                    <span class="prev swiper-button-prev items-center justify-center p-2">
                                        <svg class="neg-transition-scale" width="9" height="11"
                                            viewBox="0 0 9 13" fill="currentColor"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path fill-rule="evenodd" clip-rule="evenodd"
